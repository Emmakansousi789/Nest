...
- [ ] **Set Vercel environment variables** — After confirming the build works locally, go to Vercel → Settings → Environment Variables and set DATABASE_URL, NEXTAUTH_SECRET, and NEXTAUTH_URL with the new values. Then redeploy.
...
- **Apple Sign In JWT not verified** — ✅ Fixed. Uses `jose.jwtVerify()` against Apple's JWKS endpoint (commit `a6ac159`).

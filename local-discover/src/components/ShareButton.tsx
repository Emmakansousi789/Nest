...
import { shareVendor } from "@/lib/share";
...
export default function ShareButton({ vendor }: ShareButtonProps) {
  const [copied, setCopied] = useState(false);

  const handleShare = async (e: React.MouseEvent) => {
    e.preventDefault(); // Don't navigate to vendor page
    e.stopPropagation();

    const success = await shareVendor({
      vendorId: vendor.id,

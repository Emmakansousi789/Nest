...
describe("Vendor data integrity", () => {
  it("all vendors have required fields", () => {
    const vendors = [
      makeVendor({ id: "v1", name: "Farm", category: "farmers-market" }),
      makeVendor({ id: "v2", name: "Bakery", category: "food-producer" }),
      makeVendor({ id: "v3", name: "Studio", category: "artisan" }),
    ];
    ...
    for (const v of vendors) {
      ...
      expect(v.hours).toBeTypeOf("object");
    }
  });

  it("featured vendors exist", () => {
    const vendors = [
      makeVendor({ id: "v1", featured: true }),
      makeVendor({ id: "v2", featured: false }),
    ];
    expect(vendors.filter((v) => v.featured)).toHaveLength(1);
  });
});

describe("Search and filter edge cases", () => {
  it("handles empty vendor list", () => {
    expect(filterVendors([], "all", [], "")).toHaveLength(0);
  });

  it("handles case-insensitive search", () => {
    const vendors = [makeVendor({ name: "Upper Case Shop" })];
    expect(filterVendors(vendors, "all", [], "upper")).toHaveLength(1);
    expect(filterVendors(vendors, "all", [], "UPPER")).toHaveLength(1);
  });

  it("handles multiple tag filters (OR logic)", () => {
    const vendors = [
      makeVendor({ id: "v1", tags: ["organic"] }),
      makeVendor({ id: "v2", tags: ["handmade"] }),
      makeVendor({ id: "v3", tags: ["vintage"] }),
    ];
    expect(filterVendors(vendors, "all", ["organic", "handmade"], "")).toHaveLength(2);
  });

  it("handles search across multiple fields", () => {
    const vendors = [
      makeVendor({
        name: "Test",
        tagline: "Great food",
        city: "Savannah",
        products: [{ id: "p1", name: "Artisan Bread", description: "", imageUrl: "", category: "Food" }],
      }),
    ];
    expect(filterVendors(vendors, "all", [], "savannah")).toHaveLength(1);
    expect(filterVendors(vendors, "all", [], "bread")).toHaveLength(1);
    expect(filterVendors(vendors, "all", [], "food")).toHaveLength(1);
  });
});
